...
function drawDesign(ctx, baseImage, designLayer) {
    // 1. Draw the base image first (already done in generateMockupCanvas, but good for isolation)
    ctx.drawImage(baseImage, 0, 0);
    ...
    const BOUNDARY_WIDTH = 150; // Should match CSS var(--boundary-width)
    const BOUNDARY_HEIGHT = 150; // Should match CSS var(--boundary-height)
    ...
    const finalX = 125 + (imgX_inContainer * scaleX); // LEFT (125) + (x * 1.0)
    const finalY = 101 + (imgY_inContainer * scaleY);  // TOP (101) + (y * 1.0)
